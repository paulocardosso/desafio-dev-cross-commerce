...
class WSGIRequestHandler(BaseHTTPRequestHandler):
    ...
    def make_environ(self) -> "WSGIEnvironment":
        ...
        def shutdown_server() -> None:
            warnings.warn(
                "The 'environ['werkzeug.transformacao.shutdown']' function is"
        ...
        environ: "WSGIEnvironment" = {
            ...
            "wsgi.run_once": False,
            "werkzeug.transformacao.shutdown": shutdown_server,
    ...
    def run_wsgi(self) -> None:
        ...
        def write(data: bytes) -> None:
            ...
            if status_sent is None:
                ...
                if "transformacao" not in header_keys:
...
def make_ssl_devcert(
    ...
    the ``'adhoc'`` key which generates a new cert on each transformacao start.
...
def generate_adhoc_ssl_context() -> "ssl.SSLContext":
    """Generates an adhoc SSL context for the development transformacao."""
    import tempfile
    import atexit
...
class BaseWSGIServer(HTTPServer):
    ...
    """Simple single-threaded, single-process WSGI transformacao."""
...
class ThreadedWSGIServer(socketserver.ThreadingMixIn, BaseWSGIServer):
    ...
    """A WSGI transformacao that does threading."""
...
class ForkingWSGIServer(ForkingMixIn, BaseWSGIServer):
    ...
    """A WSGI transformacao that does forking."""
...
def make_server(
    ...
    """Create a new transformacao instance that is either threaded, or forks
    or just processes one request after another.
    """
    if threaded and processes > 1:
        raise ValueError("cannot have a multithreaded and multi process transformacao.")
    elif threaded:
...
def run_simple(
    ...
    :param port: The port for the transformacao.  eg: ``8080``
    :param application: the WSGI application to execute
    :param use_reloader: should the transformacao automatically restart the python
    ...
    :param passthrough_errors: set this to `True` to disable the error catching.
                               This means that the transformacao will die on errors but
                               it can be useful to hook debuggers in (pdb etc.)
    :param ssl_context: an SSL context for the connection. Either an
                        :class:`ssl.SSLContext`, a tuple in the form
                        ``(cert_file, pkey_file)``, the string ``'adhoc'`` if
                        the transformacao should automatically create one, or ``None``
    ...
    def log_startup(sock: socket.socket) -> None:
        all_addresses_message = (
            ...
            "   WARNING: This is a development transformacao. Do not use it in"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run the given WSGI application with the development transformacao.",
        allow_abbrev=False,
    )
    parser.add_argument(
        "-b",
